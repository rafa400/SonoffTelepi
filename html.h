...
const char jsuijs[] PROGMEM = R"=====((function (window, document) {var layout = document.getElementById('layout'), menu = document.getElementById('menu'), menuLink = document.getElementById('menuLink'), content = document.getElementById('main');function toggleClass(element, className) { var classes = element.className.split(/s+/), length = classes.length, i = 0;for(; i < length; i++) { if (classes[i] === className) { classes.splice(i, 1); break; } } if (length === classes.length) { classes.push(className); }element.className = classes.join(' '); }function toggleAll(e) { var active = 'active';e.preventDefault(); toggleClass(layout, active); toggleClass(menu, active); toggleClass(menuLink, active); }menuLink.onclick = function (e) { toggleAll(e); };content.onclick = function(e) { if (menu.className.indexOf('active') !== -1) { toggleAll(e); } };}(this, this.document));)=====";
         ...
         <fieldset>
          ...
          <div class="pure-control-group">
            ...
            <select id="wifimode" name="wifimode">
              ...
              <option value="ADH" %a4s>Ad Hoc</option>
            </select>
          </div>
          <div class="pure-control-group">
...
<div id="layout">
    ...
    <div id="main">
       ...
       <div class="content">
       <form id="mqtthtml" class="pure-form pure-form-aligned" method="post">
         <fieldset>
        ...
        <fieldset>  <legend></legend>
          <button class="pure-button pure-button-primary">Accept</button>
